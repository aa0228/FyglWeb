...
public class getpasswd {
    ...
    /*   for(li_row=0;li_row<4;li_row++){
    	   for(li_col=0;li_col<4;li_col++){
    		  int bi=(li_row*5) + li_col+1;
    		  String s= l_xlh.substring(bi-1, bi);
    		  temp[li_row][ li_col] = s.charAt(0);
    	   }
       }
    		  ...
    		  for(li_col=0;li_col<4;li_col++){
    			  for(li_row=0;li_row<4;li_row++){
    				  l_xlh= l_xlh + temp[li_row][li_col];
    			  }
    			  if(li_col<3)
    			  l_xlh = l_xlh + "-";
		...
		for (i=0;i<lf1;i++){
			...
			l_return=l_return+l_fydm.substring(i,i+1);
			//+l_cnt.substring(l_cnt.length()-k);
		}
		for (i=0;i<lf1;i++){
			l_pos=Integer.parseInt(l_fydm.substring(i, i+1));
			//  System.out.println(l_xlh);
	...
	public static void main(String[] args)
	{
		...
		String l_return=oo.passwd("120000 200","AKWC-GT3R-774L-TKPS");
